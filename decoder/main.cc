...
int main(int argc, char * argv[])
{
    ...
    int option;
    while ((option = getopt(argc, argv, "hPDwr:t:i:o:d:f")) != -1)
    {
        switch (option)
        {
        case 'r':
        ...
        case 'D':       // select DMO mode 
            programMode |= DMO_MODE;    // DMO_MODE enabled when programMode_bit 4 = 1
            bDmoMode = true;
            break;
    ...
    printf("\n Before creating decoder object OK\n");
    // create decoder
    Tetra::TetraDecoder * decoder = new Tetra::TetraDecoder(udpSocketFd, bRemoveFillBits, logLevel, bEnableWiresharkOutput, bDmoMode);
    printf("\n Create decoder object OK\n");

    // calculate BER based on detected Preamle & Training Sequences
    decoder->getBitErrs(&totalBits, &errorBits);
    printf("Total bits measured: %lu, Errored bits: %lu\n", totalBits, errorBits);
    ...
    // calculate BER based on detected Preamle & Training Sequences
    decoder->getBitErrs(&totalBits, &errorBits);
    printf("Total bits measured: %lu, Errored bits: %lu, BER (per million bits): %lu \n", totalBits, errorBits, errorBits*1000000/totalBits);
    delete decoder;
