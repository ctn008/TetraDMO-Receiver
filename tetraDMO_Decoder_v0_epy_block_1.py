...
class lowerMac(gr.sync_block):
    ...
    def __init__(self, mode= True):
        gr.sync_block.__init__(
            self,
            name='lower-upper MAC',   # will show up in GRC
            in_sig=[np.uint8],
            out_sig=[np.uint8]
        )
        self.mode = mode
    ...
    def processDmacSync(self, pdu):
        ...
        print("TN:FN", self.m_tetraTime.tn,':', self.m_tetraTime.fn, " fragFlag: ", self.fragFlag)
        ...
        print("Scrambling code: ", hex(self.m_tetraCell.getScramblingCode()) )
        print("Source address: ", self.sourceAddress, "Destin address: ", self.destinAddress)
        
    def work(self, input_items, output_items):
        burst_index = 0
        while len(input_items[0]) - burst_index >= 520:
            print("MN:FN:TN = ", self.m_tetraTime.mn,":",self.m_tetraTime.fn,":",self.m_tetraTime.tn, " ", end ='')
            if input_items[0][burst_index] == DSB:
                bkn1 = np.array(input_items[0][burst_index+10+34+94 :burst_index+10+34+94 +120])
                bkn2 = np.array(input_items[0][burst_index+10+34+252:burst_index+10+34+252+216])
            ...
            elif input_items[0][burst_index] == DNB:
                bkn1 = np.array(input_items[0][burst_index+10+34+14 :burst_index+10+34+14 +216])
                bkn2 = np.array(input_items[0][burst_index+10+34+252:burst_index+10+34+252+216])
            ...
            elif input_items[0][burst_index] == DNB_SF:
                bkn1ValidFlag = False
                bkn2ValidFlag = False
            ...
            output_items[0][burst_index:burst_index+520] = input_items[0][burst_index:burst_index+520] 
            burst_index += 520
            self.burst_cnt += 1
