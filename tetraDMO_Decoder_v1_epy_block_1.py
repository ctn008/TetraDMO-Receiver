...
class lowerMac(gr.basic_block):
    ...
    def general_work(self, input_items, output_items):
        in_index = 0
        out_index = 0
        while len(input_items[0]) - in_index >= BURST_IN:
            ...
            if input_items[0][in_index] == DSB:
                ...
                out_index += FRAME_OUT
                # last byte of FRAME indicates 0 = normal/full speech Frame
                output_items[0][out_index-1] = 0
                ...
                output_items[0][out_index:out_index+216] = np.array([0]*216, dtype='uint8')
                
                if (self.m_sfStolenFlag):        # if second-half slot is also stolen
                    output_items[0][out_index+216:out_index+432] = np.array([0]*216, dtype='uint8')
                    if (bkn2ValidFlag):
                    ...
                    output_items[0][out_index+216:out_index+432] = bkn2_s[:]
                    out_index += len(bkn2)
                out_index += FRAME_OUT
                # last byte of FRAME indicates 1 = stolen Frame
                output_items[0][out_index-1] = 1
            else:
                print("Inactive")
